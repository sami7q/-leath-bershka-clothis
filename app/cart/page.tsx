...
export default function CartPage() {
  ...
  const waLink = useMemo(() => {
    ...
    items.forEach((x, i) => {
      ...
      const sizePart = x.size ? ` (Size: ${x.size})` : "";
      lines.push(`${i + 1}) ${x.name}${sizePart} ×${x.qty} — ${currency}${lineTotal.toFixed(0)}`);
    });
      ...
      <div className="mx-auto max-w-5xl px-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-[13px]" style={{ color: BRAND.textMuted }}>
              العناصر: {count} • الإجمالي: {currency}
              {total.toFixed(0)}
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_360px]">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {x.size && <p className="mt-0.5 text-[11px] font-bold text-white/65">Size: {x.size}</p>}
          ...
          <aside className="rounded-2xl border p-4" style={{ borderColor: BRAND.border, background: BRAND.card }}>
            ...
            {/* ✅ FIX: keep number LTR inside Arabic */}
            <p className="mt-3 text-[10px] font-bold text-white/45">
              الدعم: <span dir="ltr">+90 501 595 4826</span>
            </p>
          </aside>
        </div>
      </div>
